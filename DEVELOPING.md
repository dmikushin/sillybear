...
Set `#define DEBUG_TRACE 1` in [localoptions.h](./localoptions.h) to enable a `-v` verbose option for sillybear and dbclient.
Higher numbers can be used to allow increased debug levels, with `-v` argument repeated.

For development running `sillybear -F -E` is useful to run in the foreground.
You can set `#define DEBUG_NOFORK 1` to make sillybear a one-shot server, easy to run under a debugger.

#### Random sources

Most cryptography requires a good random entropy source, both to generate secret keys and in the course of a session.
Sillybear uses the Linux kernel's `getrandom()` syscall to ensure that the system RNG has been initialised before using it.
...
Sillybear has a `-R` option to generate hostkeys upon the first connection as required - that will allow the system more time to gather entropy.

#### Algorithms

Default algorithm lists are specified in [common-algo.c](./src/common-algo.c). They are in priority order, the client's first matching choice is used (see [rfc4253](https://www.rfc-editor.org/rfc/rfc4253.html)). Sillybear client has `-c` and `-m` arguments to choose which are enabled at runtime (doesn't work for server as of June 2020).
...
Some Sillybear platforms have old compilers.
...
Sillybear server will run fine as a non-root user, allowing logins only for that user.
...
Sillybear implements `first_kex_packet_follows` to reduce handshake latency [RFC 4253 7.1](https://www.rfc-editor.org/rfc/rfc4253.html#section-7.1).
Some less common implementations don't handle that - it can be a cause of problems connecting.
Note also that Sillybear may send several ssh packets within a single TCP packet - it's just a stream.
