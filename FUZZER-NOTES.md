...
Sillybear is process-per-session so it assumes calling `sillybear_exit()` is fine at any point to clean up.
...
When sillybear runs in fuzz mode it sets up a [`setjmp()`](http://man7.org/linux/man-pages/man3/setjmp.3.html) target prior to launching the code to be fuzzed, and then [`sillybear_exit()`](./src/dbutil.c#L125) calls `longjmp()` back there.
This avoids exiting though it doesn't free memory or other resources.

#### malloc Wrapper

Sillybear normally uses a [`m_malloc()`](./src/dbmalloc.c) function that is the same as `malloc()` but exits if allocation fails.
...
As a network process Sillybear reads and writes from a socket.
...
This allows running the entire sillybear server process with network input provided by the fuzzer, without many modifications to the main code.
...
When running in fuzzing mode Sillybear uses a [fixed seed](./src/dbrandom.c#L185) every time so that failures can be reproduced.
...
* [fuzzer-kexcurve25519](./fuzz/fuzzer-kexcurve25519.c) - test Curve25519 Elliptic Curve Diffie-Hellman key exchange like fuzzer-kexecdh.
  This is testing `sillybear_curve25519_scalarmult()` and other libtommath routines.
