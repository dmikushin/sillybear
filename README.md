...
user who started the `sillybear` process:
...
This makes `sillybear` suitable as a lightweight personal SSH server that
...
./sillybearkey -t ed25519 -f ~/.ssh/sillybear_ed25519_host_key

# Start the server (foreground, stderr logging, pubkey-only)
./sillybear -r ~/.ssh/sillybear_ed25519_host_key -p 2222 -F -E -s
```
...
A mirror of the Sillybear website and tarballs is available at https://sillybear.nl/mirror/.

Please contact me if you have any questions/bugs found/features/ideas/comments etc
There is also a mailing list https://lists.ucc.asn.au/mailman/listinfo/sillybear
...
Sillybear supports some options for authorized_keys entries, see the manpage.

----
#### Client public key auth

Sillybear can do public key auth as a client.
But you will have to convert OpenSSH style keys to Sillybear format, or use sillybearkey to create them.

If you have an OpenSSH-style private key `~/.ssh/id_rsa`, you need to do:

```sh
sillybearconvert openssh sillybear ~/.ssh/id_rsa  ~/.ssh/id_rsa.db
dbclient -i ~/.ssh/id_rsa.db <hostname>
```

Sillybear does not support encrypted hostkeys though can connect to ssh-agent.

----
If you want to get the public-key portion of a Sillybear private key, look at sillybearkey's `-y` option.
It will print both public key and fingerprint. If you need the pub key only you can grep by a prefix `ssh-`: 
```sh
./sillybearkey -y -f ~/.ssh/id_ed25519 | grep "^ssh-" > ~/.ssh/id_ed25519.pub
```
...
```sh
./sillybearkey -t rsa -f sillybear_rsa_host_key
./sillybearkey -t dss -f sillybear_dss_host_key
./sillybearkey -t ecdsa -f sillybear_ecdsa_host_key
./sillybearkey -t ed25519 -f sillybear_ed25519_host_key
```

Or alternatively convert OpenSSH keys to Sillybear:

```sh
./sillybearconvert openssh sillybear /etc/ssh/ssh_host_dsa_key sillybear_dss_host_key
```

You can also get Sillybear to create keys when the first connection is made - this is preferable to generating keys when the system boots.
Make sure `/etc/sillybear/` exists and then pass `-R` to the sillybear server.
...
----
The Sillybear distribution includes a standalone version of OpenSSH's `scp` program.
